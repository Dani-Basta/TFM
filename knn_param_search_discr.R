...
#' @export
knn_param_search_discr <- function(y, k, d, indexes = NULL, distance = "euclidean",
  ...
  indexes <- sort.int(x = indexes, decreasing = FALSE)

  j_values <- n - indexes + 2

  real_values <- matrix(y[indexes, v])
  ...
  errors_matrix <- foreach(i = 1:ds, .combine = cbind,
    ...
    predictions <- matrix(nrow = ks, ncol = length(real_values))
    ...
    for (j_index in 1:length(j_values)) {
      j <- j_values[j_index]
  ...
  index_min_error <- which(errors_matrix == min(errors_matrix), arr.ind = TRUE)
  opt_k <- k[index_min_error[1]]
  opt_d <- d[index_min_error[2]]
