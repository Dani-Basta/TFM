...
pNeighStdevOptim <- plot_ly(name = "Future values Std.Dev.", showlegend = TRUE, hoverinfo = "x+y",
...
pScatDistErrOptim <- plot_ly(name = "Distances vs Errors", type = 'scatter', mode = "lines", hoverinfo = "x+y",
  ...
  add_trace(y = (loess_mean_DisErr$fit - loess_ci_DisErr)[loess_ind_DisErr]) %>%
  add_trace(y = (loess_mean_DisErr$fit + loess_ci_DisErr)[loess_ind_DisErr], 
...
pScatStddevErrOptim <- plot_ly(name = "Std. Dev. vs Errors", type = 'scatter', mode = "lines", hoverinfo = "x+y",
  ...
  add_trace(y = (loess_mean_DevErr$fit - loess_ci_DevErr)[loess_ind_DevErr]) %>%
  add_trace(y = (loess_mean_DevErr$fit + loess_ci_DevErr)[loess_ind_DevErr], 
            fill = "tonexty", fillcolor = "rgba(0,0,100,0.35)") %>%
  add_trace(x = neighs_stdev, y = abs(residuals_matrix[1, ]), mode = "markers", marker = list(opacity = 0.4),
            hoverinfo = "x+y+text", text = head(tail(dates, length(y) + 1 - train_init), length(sub_dates))) %>%
  layout(xaxis = list(title = "Standard Deviation"), yaxis = list(title = "Errors"))
...
pOpt <- plot_ly(x = dates, y = y, type = "scatter",  name = "Observed Time Series", 
  ...
  add_trace(x = sub_dates, y = optimal$fitted, legendgroup = "optim", 
            name = paste0("Optimal (k = ", res$opt_k, ", d = ", res$opt_d, ")"), line = list(color = colPalette[2])) %>%
  add_segments(x = dates[train_init], xend = dates[train_init], y = min_y - 0.05 * (max_y - min_y), 
  ...
  add_segments(x = dates[test_init], xend = dates[test_init], y = min_y - 0.05 * (max_y - min_y), 
...
pContourBase <- plot_ly(x = ks , y = ds, z = res$errors, transpose = TRUE, type = "contour", source = "contour", 
                        colorscale = "Rainbow", contours = list(showlabels = TRUE, coloring = "heatmap", 
                                                            start = cont_min, end = cont_max,
                                                            size = (cont_max - cont_min)/(4*num_contours)),
                        hoverinfo = "x+y+z") %>%
  layout(xaxis = list(title = "k"), yaxis = list(title = "d") ) %>%
  add_trace(type = "scatter", mode = "markers", x = x_minims[1], y = y_minims[1],
            text = paste0("Best combination\n", as.character( signif(res$errors[x_minims[1], y_minims[1] ], digits = 4) ) ),
...
pDisDevHeatOptim <- plot_ly(x = loess_distances, y = loess_deviations, z = loess_DisDev_heatmap, type = "contour", 
  ...
  add_trace(x = optimal$knn_dists, y = neighs_stdev, type = "scatter", mode = "markers", hoverinfo = "text",
            marker = list(color = "rgba(0,0,0,0.7)", size = 3, line = list(color = "rgba(255,255,255,0.7)",  width = 1))) %>%
  layout(xaxis = list(title = "Distance"), yaxis = list(title = "Standard Deviation") ) %>% colorbar(title = "Error")
...
#Default
pManualComb <- plot_ly(x = ks , y = ds, z = res$errors, transpose = TRUE, type = "contour", source = "manual", 
                        colorscale = "Rainbow", contours = list(showlabels = TRUE, coloring = "heatmap", 
                                                              start = cont_min, end = cont_max,
                                                              size = (cont_max - cont_min)/(4*num_contours)),
                        hoverinfo = "x+y+z") %>% layout(xaxis = list(title = "k"), yaxis = list(title = "d") ) %>% colorbar(title = "Error")
